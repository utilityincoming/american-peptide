    ...
    <div className="min-h-screen bg-[#0B1220] text-white">
      <header className="flex items-center gap-2 border-b border-white/[0.06] px-4 py-3 md:px-6">
        <span className="flex items-center gap-1.5 text-sm font-medium">
          <Star className="h-4 w-4 text-[#2DD4A8]" />
          Workspace
        </span>
      </header>
        ...
        <div className="mx-auto max-w-4xl space-y-10">
          {hasNothing && (
            <div className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-10 text-center">
              ...
              <p className="mb-1 text-sm font-medium text-white/70">Your workspace is empty</p>
              <p className="mx-auto mb-5 max-w-md text-sm text-white/45">
              ...
              <div className="flex flex-wrap justify-center gap-3">
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 px-5 py-2.5 text-sm font-medium text-white/60 hover:border-white/20 hover:text-white"
                >
            ...
            <div>
              ...
              <div className="space-y-5">
                {slugs.map((slug) => {
                  ...
                  return (
                    <div key={slug} className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-5">
                      <div className="mb-3 flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2.5">
                          <Link
                            href={`/catalog/${slug}`}
                            className="text-base font-semibold text-white/90 hover:text-[#2DD4A8]"
                          >
                            {wp?.name ?? slug}
                          </Link>
                          {newCount > 0 && (
                            <span className="rounded-full border border-[#2DD4A8]/30 bg-[#2DD4A8]/[0.10] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-[#2DD4A8]">
                              {newCount} new
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-3">
                          <Link
                            ...
                            className="hidden items-center gap-1 text-xs text-white/40 hover:text-[#2DD4A8] sm:inline-flex"
                          >
                      ...
                      {loading && !wp ? (
                        <p className="text-xs text-white/35">Loading latest research…</p>
                      ) : items.length === 0 ? (
                        <p className="text-xs text-white/35">No recent trials or publications found.</p>
                      ) : (
                        <>
                            ...
                            {items.slice(0, 8).map((i) => {
                              ...
                              return (
                                <li key={i.id}>
                                  ...
                                  >
                                    <Icon className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#2DD4A8]/60" />
                                    <span className="min-w-0 flex-1">
                                      <span className="block text-sm leading-snug text-white/75 group-hover:text-white/90">
                                        {isNew && <span className="mr-1.5 text-[#2DD4A8]">●</span>}
                                        {i.title}
                                      </span>
                                      <span className="block text-[11px] text-white/35">{i.meta}</span>
                                    </span>
                                  </a>
                                </li>
                              )
                          ...
                          {newCount > 0 && (
                            <button
                              ...
                              className="mt-3 inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-1.5 text-xs font-medium text-white/55 transition-colors hover:border-white/20 hover:text-white"
                            >
            ...
            <div>
              ...
              <div className="space-y-2">
                {designs.map((d) => (
                  ...
                  >
                    <code className="min-w-0 flex-1 truncate font-mono text-sm text-white/75">{d.seq}</code>
                    <Link
                      href={`/tools/design-lab?s=${encodeURIComponent(d.seq)}`}
                      className="inline-flex shrink-0 items-center gap-1 text-xs font-medium text-[#2DD4A8] hover:underline"
                    >
