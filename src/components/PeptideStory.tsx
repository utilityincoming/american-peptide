...
export default function PeptideStory({ peptide }: { peptide: Peptide }) {
  ...
  return (
    <div className="rounded-2xl border border-[#2DD4A8]/15 bg-gradient-to-br from-[#2DD4A8]/[0.05] to-transparent p-6 md:p-7">
      ...
      <p className="mb-6 text-sm leading-relaxed text-ink/65">{story.intro}</p>

      <ol className="relative space-y-5 border-l border-ink/[0.08] pl-6">
        {story.beats.map((beat, i) => {
          const Icon = ICONS[beat.icon] ?? Cpu
          return (
            <li key={i} className="relative">
              <span className="absolute -left-[2.05rem] flex h-7 w-7 items-center justify-center rounded-lg border border-[#2DD4A8]/25 bg-surface text-accent">
                <Icon className="h-3.5 w-3.5" strokeWidth={1.75} />
              </span>
              <Link
                href={`/synthesis#${beat.stage}`}
                className="group inline-flex items-center gap-1.5 text-[13px] font-semibold tracking-tight text-ink/85 transition-colors hover:text-accent"
              >
                {beat.title}
      ...
      <Link
        href="/synthesis"
        className="group mt-6 inline-flex items-center gap-1.5 text-xs font-medium text-accent"
      >
